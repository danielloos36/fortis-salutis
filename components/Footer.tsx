import Link from 'next/link'
import Image from 'next/image'

export default function Footer() {
  return (
    <footer className="bg-[#0a0a0a] text-white">
      <div className="max-w-7xl mx-auto px-6 py-16 lg:py-20">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          {/* Brand */}
          <div className="sm:col-span-2 lg:col-span-1">
            <div className="flex items-center gap-2.5 mb-5">
              <Image
                src="/logo.png"
                alt="Fortis Salutis"
                width={32}
                height={32}
                className="h-8 w-auto invert brightness-0 invert"
              />
              <span className="font-black text-base">Fortis Salutis</span>
            </div>
            <p className="text-gray-500 text-sm leading-relaxed mb-5">
              Physiotherapie in Reinheim, Hessen. Individuelle Behandlung
              auf höchstem Niveau – mehr Zeit, mehr Fürsorge.
            </p>
            <address className="not-italic text-sm text-gray-500 space-y-1">
              <p>Darmstädter Straße 43</p>
              <p>64354 Reinheim</p>
            </address>
          </div>

          {/* Leistungen */}
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-[0.25em] text-gray-600 mb-5">
              Leistungen
            </h3>
            <ul className="space-y-2.5">
              {[
                ['Krankengymnastik', '/leistungen/krankengymnastik'],
                ['Trainingstherapie', '/leistungen/trainingstherapie'],
                ['Manuelle Therapie', '/leistungen/manuelle-therapie'],
                ['Neurologische Therapie', '/leistungen/neurologische-therapie'],
                ['Hausbesuche', '/leistungen/hausbesuche'],
              ].map(([title, href]) => (
                <li key={href}>
                  <Link
                    href={href}
                    className="text-sm text-gray-500 hover:text-white transition-colors"
                  >
                    {title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Einzugsgebiet */}
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-[0.25em] text-gray-600 mb-5">
              Einzugsgebiet
            </h3>
            <ul className="space-y-2.5">
              {[
                ['Reinheim', '/standorte/reinheim'],
                ['Groß-Bieberau', '/standorte/gross-bieberau'],
                ['Dieburg', '/standorte/dieburg'],
                ['Ober-Ramstadt', '/standorte/ober-ramstadt'],
                ['Darmstadt', '/standorte/darmstadt'],
                ['Otzberg', '/standorte/otzberg'],
                ['Roßdorf', '/standorte/rossdorf'],
              ].map(([title, href]) => (
                <li key={href}>
                  <Link
                    href={href}
                    className="text-sm text-gray-500 hover:text-white transition-colors"
                  >
                    {title}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Kontakt */}
          <div>
            <h3 className="text-[10px] font-bold uppercase tracking-[0.25em] text-gray-600 mb-5">
              Kontakt
            </h3>
            <div className="space-y-3 text-sm">
              <a
                href="tel:+4915773327200"
                className="flex items-center gap-2.5 text-gray-500 hover:text-white transition-colors"
              >
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.948V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                </svg>
                +49 157 73327200
              </a>
              <a
                href="mailto:info@fortis-salutis.de"
                className="flex items-center gap-2.5 text-gray-500 hover:text-white transition-colors"
              >
                <svg className="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                </svg>
                info@fortis-salutis.de
              </a>
              <a
                href="https://wa.me/4915773327200"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2.5 text-gray-500 hover:text-white transition-colors"
              >
                <svg className="w-4 h-4 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                WhatsApp
              </a>
            </div>
          </div>
        </div>

        <div className="border-t border-gray-900 mt-14 pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
          <p className="text-gray-700 text-xs">
            © {new Date().getFullYear()} Fortis Salutis – Physiotherapie Reinheim. Alle Rechte vorbehalten.
          </p>
          <div className="flex gap-6">
            <Link href="/impressum" className="text-gray-700 hover:text-gray-400 text-xs transition-colors">
              Impressum
            </Link>
            <Link href="/datenschutz" className="text-gray-700 hover:text-gray-400 text-xs transition-colors">
              Datenschutz
            </Link>
          </div>
        </div>
      </div>
    </footer>
  )
}
